export async function fetchArchive(assetIds: string[], opts?: { filename?: string; onProgress?: (progress: number) => void; signal?: AbortSignal }): Promise<Blob> {
  ...
  for (const url of candidates) {
    try {
      ...
      const isBinary = contentType.includes('zip') || contentType.includes('octet-stream') || contentType.startsWith('application/') || contentType.startsWith('image/') || contentType.startsWith('video/');
        ...
        if (onProgress && res.body && typeof ReadableStream !== 'undefined') {
          ...
          while (true) {
            ...
            try {
              ...
              // If aborted, propagate a DOMException with name 'AbortError'
              throw err;
            }
