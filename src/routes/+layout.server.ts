    ...
    try {
        ...
        if (cookieSigned) {
            ...
            if (verified) {
                ...
                if (userInfo) return { session: { user: userInfo } };
            }
        ...
        const providerUser: any = session.user;
        ...
        let userInfo = stmt.get(candidateId) as User | null;

        // Si non trouvé par id_user, tenter par email si fourni
        if (!userInfo && providerUser.email) {
            stmt = db.prepare("SELECT * FROM users WHERE email = ? LIMIT 1");
            userInfo = stmt.get(providerUser.email) as User | null;
        }
        ...
        if (!userInfo) {
            ...
            userInfo = db.prepare("SELECT * FROM users WHERE id_user = ? LIMIT 1").get(candidateId) as User | null;
        }
        ...
        try {
            console.log('[session] provider id:', providerUser?.id || providerUser?.preferred_username || providerUser?.sub, 'email:', providerUser?.email);
            console.log('[session] mapped local user:', { id_user: userInfo.id_user, role: userInfo.role });
        } catch (e) {
            // ignore
        }
