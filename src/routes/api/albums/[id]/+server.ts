  ...
  try {
    ...
    if (tags && Array.isArray(tags)) {
      ...
      for (const tag of tagsToAdd) {
        db.prepare(
          "INSERT INTO album_permissions (album_id, kind, value) VALUES (?, 'tag', ?)"
        ).run(id, tag);
      }
    ...
    if (allowedUsers && Array.isArray(allowedUsers)) {
      ...
      for (const userId of usersToAdd) {
        db.prepare(
          "INSERT INTO album_permissions (album_id, kind, value) VALUES (?, 'user', ?)"
        ).run(id, userId);
      }
    ...
    if (Array.isArray(formations)) {
      ...
      for (const formation of formationsToAdd) {
        db.prepare(
          "INSERT INTO album_permissions (album_id, kind, value) VALUES (?, 'formation', ?)"
        ).run(id, formation);
      }
    ...
    if (Array.isArray(promos) || Array.isArray(tags)) {
      ...
      for (const promo of promosToAdd) {
        db.prepare(
          "INSERT INTO album_permissions (album_id, kind, value) VALUES (?, 'promo', ?)"
        ).run(id, String(promo));
      }

      const promosToRemove = existingPromoYears.filter(
        (promo) => !normalizedPromos.includes(promo)
      );
      for (const promo of promosToRemove) {
        db.prepare(
          "DELETE FROM album_permissions WHERE album_id = ? AND kind = 'promo' AND value = ?"
        ).run(id, String(promo));
      }
    }
