...
export const GET: RequestHandler = async (event) => {
	try {
		...
		try {
			...
			console.warn('[asset-thumbnail/thumbnail] failed to read local DB visibility', dbErr);
		}
		...
		if (!proxied.ok) {
			...
			console.error('[asset-thumbnail/thumbnail] upstream returned non-ok', {
				assetId,
		...
		console.error('[asset-thumbnail/thumbnail] error', err);
