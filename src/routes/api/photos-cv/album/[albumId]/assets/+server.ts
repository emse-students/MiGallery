...
async function isAuthorized(request: Request, locals: any, cookies: any, requiredScope?: string) {
  // Prefer API key
  ...
  if (role === 'admin') return { by: 'user', user };
  if (requiredScope === 'write' && role === 'mitviste') return { by: 'user', user };
