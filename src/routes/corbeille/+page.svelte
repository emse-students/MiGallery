  ...
  async function fetchTrashedAssets() {
    ...
    try {
      ...
      for (const bucket of buckets) {
        try {
          const bucketRes = await fetch(`/api/immich/timeline/bucket?timeBucket=${bucket.timeBucket}&isTrashed=true&size=MONTH`);
          if (!bucketRes.ok) {
    ...
    {#if selecting && selectedAssets.length > 0}
      <div class="selection-toolbar">
        ...
        <div class="selection-actions">
          ...
          <button onclick={() => restoreAssets(selectedAssets)} class="px-2 py-2 bg-emerald-600 hover:bg-emerald-700">
            <Icon name="arrow-left" size={16} />
            Restaurer ({selectedAssets.length})
          </button>
          <button onclick={() => deleteAssetsPermanently(selectedAssets)} class="btn-delete-selection px-2 py-2 text-white">
      ...
      <div class="photos-flex {selecting ? 'selection-mode' : ''}">
        {#each items as asset}
          ...
          <div
            ...
            onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handlePhotoClick(asset.id, e); } }}
          >
            <div class="selection-checkbox {selectedAssets.includes(asset.id) ? 'checked' : ''}">
              <input type="checkbox" checked={selectedAssets.includes(asset.id)} onchange={(e) => toggleSelect(asset.id, (e.target as HTMLInputElement).checked)} onclick={(e) => e.stopPropagation()} />
            </div>

            {#if !selecting}
              <button class="restore-btn" title="Restaurer" onclick={(e) => { e.stopPropagation(); restoreAssets([asset.id]); }}>
                <Icon name="arrow-left" size={16} />
              </button>
              <button class="delete-permanent-btn" title="Supprimer définitivement" onclick={(e) => { e.stopPropagation(); deleteAssetsPermanently([asset.id]); }}>
                <Icon name="trash" size={16} />
              </button>
            {/if}
