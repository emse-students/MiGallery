<script lang="ts">
  ...
  const pickerAlbums = $derived(fuzzySearch(albums, pickerQuery, (a) => a.albumName).slice(0, 40));
...
<AdminPage title={m.med_title()} subtitle={m.med_subtitle()} icon={Images} maxWidth="1100px">
  {#snippet actions()}
      ...
      {:else}
        ...
        {#if pickerOpen}
          <div class="picker">
            ...
            <input type="text" bind:value={pickerQuery} placeholder={m.med_album_pick_search()} />
        ...
        <div class="pager">
          <button
            ...
            onclick={() => (viewStart = Math.max(0, viewStart - VIEW_SIZE))}
          >
            &larr;
          </button>
          <span>
