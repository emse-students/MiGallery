...
- \[ \] WP-5 "vignettes ne s'affichent pas": BACKEND PROVEN HEALTHY (2026-07-13 prod diag, scratchpad wp5-diag.mjs). Immich returns thumbnail?size=thumbnail 200 image/webp (valid, viewable), size=preview 200, /api/faces 200 with full expected shape (boundingBoxX1..Y2, imageWidth/Height, person.id). Same thumbnails render everywhere else -> proxy+auth+path OK in-browser. Bug is CLIENT-SIDE in FaceCropThumb.svelte only. Suspects: .face-crop height:100% collapsing (parent .photo-item is aspect-ratio:1 button), or IntersectionObserver never setting inView (img is {#if inView}, so skeleton stays forever), or the ready=imgLoaded&&boxResolved gate. NEXT: browser DevTools (Network status of size=thumbnail req, Console errors, computed height of .face-crop). Only THEN edit.
