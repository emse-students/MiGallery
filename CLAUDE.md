...
- \[ \] WP-5.x remaining: face-first profile picker (profile-photo selector driven by detected faces, likely around ChangePhotoModal.svelte). Clean [TEST] albums = DONE/verified 2026-07-12 (prod already clean: 0 real test albums out of 266; regex false-positive was the real album "Soiree The Greatest Show'Mines"). Immich prod access recipe: pipe a node script into `ssh mitv "docker exec -i migallery-migallery-1 node"` (container has global fetch + IMMICH_BASE_URL/IMMICH_API_KEY, header x-api-key).
