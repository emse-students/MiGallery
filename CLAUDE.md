...
- Next step: Theme 4 backlog (UX / Immich / logger / locale) is the only open work. Pick one item before starting (they are independent WPs). FR#3 upload resilience DONE this session (c49e2c6). PROD DEPLOY NOTE: WP-3a's DROP runs on prod at next deploy; prod DB already backed up on mitv (migallery_backup_pre-wp3a_20260712_020159.db).
