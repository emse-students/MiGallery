...
- Next step: Theme 4 backlog (UX / logger / locale) + WP-5.x remaining (face-first profile picker) are the open work. Pick one (independent WPs). FR#3 upload resilience DONE this session (c49e2c6); WP-5.1 combined queries DONE this session (283d4f4). PROD DEPLOY NOTE: WP-3a's DROP runs on prod at next deploy; prod DB already backed up on mitv (migallery_backup_pre-wp3a_20260712_020159.db).
...
- \[~\] WP-5.x face-first profile picker: CODE WRITTEN, NOT YET type-checked/committed (session ended on the classifier-unavailable signal before `npm run check` + commit could run). Changes: new src/lib/components/FaceCropThumb.svelte (lazy IntersectionObserver -> GET /api/immich/faces?id={assetId}, finds face where person.id===personId, CSS-crops the thumbnail onto the bounding box via width/left/top %, kCover clamp so no gaps, center-cover fallback if no box); ChangePhotoModal.svelte now uses FaceCropThumb instead of LazyImage (removed the 2 :global lazy-image rules); cpm_pick reworded to "choose a face" (fr+en). NEXT SESSION: run `npm run check`, verify /faces field names against prod (probe below - AssetFaceResponseDto expected: boundingBoxX1/Y1/X2/Y2, imageWidth, imageHeight, person.id), fix if field names differ, then commit. UNVERIFIED ASSUMPTION: /faces returns those exact field names.
