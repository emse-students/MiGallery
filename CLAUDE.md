...
- WP-6 PROFILE-CROP (MiGallery owns the profile photo) - CODE COMPLETE, NOT YET `npm run check`-ED, NOT COMMITTED (shell classifier went down mid-WP = session-end signal). Next session MUST: `npm run check`, then `git add . && git commit`. Idea: stop serving Immich's tightly hard-coded person thumbnail; serve OUR wider 0.62 square crop everywhere (avatar + external API + Mes-photos header). Changes: (1) new users col `photos_asset_id` (schema.sql + database.ts migration + users.ts DBUser + types/api UserRow/User + app.d.ts session + +layout.server.ts toSessionUser + photo-access SELECT). (2) EXTRACTED crop core to `$lib/server/face-crop.ts` (generateFaceCrop(assetId, personId, fetch) -> {ok,buffer}|{ok:false,reason:busy|notfound|config|error}, disk cache data/cache/faces, OUTPUT_SIZE=320, FACE_TARGET=0.62); `/api/faces/[assetId]/[personId]` is now a thin wrapper over it. (3) `/api/users/[username]/avatar` reads photos_asset_id -> generateFaceCrop, serves webp (Cache-Control long+immutable when `?v` present, else 1h), FALLS BACK to the Immich `/people/{id}/thumbnail` proxy on any crop failure or legacy users (no photos_asset_id). (4) Persist on face pick: `PATCH /api/users/me/face` accepts optional `photos_asset_id` (sets both cols); mes-photos + parametres handlePhotoSelected now PATCH it after the Immich featureFaceAssetId PUT (best-effort, mes-photos passes user_id for admin editing others). (5) Header: photosState.loadPerson(id, {userId, version}) sets imageUrl = `/api/users/{userId}/avatar?v={photos_asset_id}` (plain URL, no blob) instead of the Immich thumbnail; 3 callers in mes-photos wired (own + own-via-param + other-via-access). GOTCHA: the 3 `<Avatar>` usages (parametres perm-lists, admin list) were intentionally NOT given ?v - the endpoint serves our crop regardless; only stale up to 1h after an edit, acceptable for other-users. Awaiting: npm run check (watch for any residual type slip), commit, then USER browser QA that the profile photo is now our wider crop everywhere.
