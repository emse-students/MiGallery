...
- \[~\] WP-5 "faces ne s'affichent pas" (ChangePhotoModal "Choisissez un visage"): backend PROVEN healthy (2026-07-13 prod diag scratchpad wp5-diag.mjs: thumbnail 200 image/webp valid+viewable, faces 200 full shape). User confirmed imgs DO load in Network but nothing renders in the modal grid; general app thumbnails fine -> client-only in FaceCropThumb. FIX APPLIED (awaiting user browser confirm): .face-crop was width/height:100% and collapsed to 0 because parent .photo-item height is aspect-ratio:1 (percentage height doesn't resolve) -> changed to position:absolute; inset:0. Also relaxed ready = imgLoaded (was imgLoaded && boxResolved), removed boxResolved, so a slow/failed face lookup no longer blanks the photo. Design (confirmed to user): Immich has NO pre-cropped face images; FaceCropThumb CSS-crops the full thumbnail using /api/faces bounding boxes - correct by design.
