...
- UI: single source of truth is `src/app.css` (tokens, `--radius-*`). dark-first, **flat**: no glow, no glassmorphism (`backdrop-filter`), no text-shadow - the user, 2026-09-25: _"les effets glow ... c'est pas du tout l'esprit des app de 2026"_, measured at 0 on Google Photos against 30 glass surfaces here ([ui-redesign](docs/wiki/ui-redesign.md)); `.btn-glass` is being retired, not extended; the soft blobs in the page BACKGROUND stay - they are the identity (user, same day); no raw hex/px, `@lucide/svelte` only - NEVER `lucide-svelte`, which is the deprecated name that still resolves and still renders (CI greps for it).
...
**Current WIP:** the Google Photos-shaped UI redesign (user, 2026-09-25). The work list, the measured reference and the two decisions owed are ONLY in [ui-redesign](docs/wiki/ui-redesign.md) - work from it, delete each row as it ships.
...
- Outside-click close can never be judged from `click` alone (its target is the common ancestor of press and release). `Modal.svelte` requires pointerdown AND pointerup on the backdrop. Do not simplify back. (The viewer has no backdrop since it went full screen.)
...
- `/mes-photos`: favorites stay in chronological place (`.favorite-badge` in `PhotoCard` + a "Toutes/Favoris" chip in `PhotosGrid`). The lightbox navigates `displayedAssets`; never reintroduce a favorites-first reordering. A long-press on a touch tile ENTERS selection mode (the old `.action-sheet` is deleted, D9); its release is `preventDefault`ed on `touchend` or the click un-picks the tile. Selection transitions live in `src/lib/selection.ts`, sharing originals in `src/lib/share-files.ts` (viewer + selection) - see `docs/wiki/photo-grid.md`.
...
- **A RUNTIME SWAP MUST FOLLOW EVERY INTERPRETER THE APP NAMES, NOT JUST THE ONE THAT STARTS IT.** `/api/admin/db-inspect` spawns `scripts/inspect-db.cjs` as a child, and the swap left that spawn on the literal `'node'`, which cannot resolve `bun:sqlite` - the endpoint then reported `success: false` for a perfectly healthy database. It is `process.execPath` now: the child runs on the interpreter the server is running on, decided by the process rather than by PATH. Before declaring a runtime migration done, grep for `spawn`, `exec` and `execFile`.
...
- `bun run test` is hermetic: it never reads `.env` (`bun --no-env-file` all the way down), runs on a per-run temp DB and an EMPTY `IMMICH_BASE_URL`, and `openTestDatabase` opens only `MIGALLERY_TEST_DATABASE`. Never let it inherit a dev `DATABASE_PATH` or an Immich URL: a `localhost` tunnel to prod Immich passed the old guard and got `[TEST]` albums created in prod ([tests/README](tests/README.md)). The server does not load `.env` itself any more (no `dotenv` in `hooks.server.ts`) - bun loads it for `bun run dev`, compose passes env in prod. Server-free suites: `tests/disk-cache.test.ts`, `tests/auth-redirect.test.ts`.
