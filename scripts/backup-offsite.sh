...
# The paths and retentions are LOCAL settings and must expand here, not on the remote
# shell - that is what the single quotes inside the double-quoted command are for.
...
# Same as above: LOCAL settings, meant to reach the remote shell already expanded.
# shellcheck disable=SC2029
...
if [ "$INCLUDE_IMMICH" = "1" ]; then
  if [ -d "$IMMICH_BACKUP_DIR" ]; then
    # The names are generated by this very script, so `ls -1t` is safe and is the
    # shortest way to say "newest".
